...
from recordings.recording_utils import generate_reording_path
load_dotenv()
...
async def entrypoint(ctx: agents.JobContext):
    ...
    def on_room_disconnected():
        ...
        try:
            logger.info("🔴[ROOM DISCONNECT] Background audio will stop automatically")
        except Exception as e:
        ...
        # Trigger cleanup and logging as backup (in case participant disconnect didn't trigger it)
        asyncio.create_task(cleanup_and_log())
        logger.info("🔴[ROOM DISCONNECT] Triggered cleanup and logging as backup")
    ...
    def on_participant_disconnected(participant):
        ...
        if len(ctx.room.remote_participants) == 0:
            ...
            try:
                logger.info("🔴[ALL PARTICIPANTS LEFT] Background audio will stop automatically")
            except Exception as e:
            ...
            # Trigger cleanup and logging since shutdown callback won't be called
            asyncio.create_task(cleanup_and_log())
            logger.info("🔴[ALL PARTICIPANTS LEFT] Triggered cleanup and logging")
    ...
    async def log_usage(starting_time, call_sid, phone_number, x_call_id):
        ...
        try:
            logger.info("📊 Attempting MongoDB insert operation...")
            result = costlogs_collection.insert_one(mongo_doc)
            logger.info(f"✅ Database operation successful! Inserted ID: {result.inserted_id}")
            logger.info(f"✅ MongoDB document created for call: {call_sid}")
        except Exception as e:
            logger.error(f"❌ Database operation failed for call {call_sid}: {e}")
            logger.error(f"❌ MongoDB connection string: {MONGODB_URI}")
            logger.error(f"❌ Collection: {costlogs_collection}")
            import traceback
    ...
    async def cleanup_and_log():
        ...
        if cleanup_called:
            logger.info("Cleanup already called, skipping duplicate cleanup")
            return
        cleanup_called = True
        logger.info("🧹 Starting cleanup and logging process")
        logger.info(f"🧹 Call SID: {call_sid}")
        logger.info(f"🧹 Caller: {caller}")
        logger.info(f"🧹 X-Call-ID: {x_call_id if x_call_id else 'Not available'}")
        ...
        try:
            logger.info("Background audio player will stop automatically when session ends")
        except Exception as e:
