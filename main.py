...
async def entrypoint(ctx: agents.JobContext):
    ...
    def handle_dtmf(dtmf: SipDTMF):
        digit = dtmf.digit
        logger.info(f"DTMF recieved from sip: {digit}")
        if digit == "0":
            asyncio.create_task(transfer_call_dtmf(agent.room.name))
        if digit == "1":
            asyncio.create_task(transfer_call_dtmf_driver(agent.room.name))
